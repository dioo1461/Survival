...
public class SlotDataMGR : MonoBehaviour
{
	...
	*/
	...
	/// <summary> 지정한 슬롯을 인벤토리의 목표 index에 복사.<br/>
	/// amount를 지정하지 않으면 amount도 복사하여 옮김.
	/// </summary>
	...
	public int Add_Item(ItemProp itemProp, int amount = -1, int index = -1) {
		...
		if (index == -1) { // index 지정하지 않으면, 인벤토리 내에서 같은 종류의 아이템 탐색
			...
			if (_sameItemIndex != -1) { // 같은 종류의 아이템 존재하면, 해당 슬롯에 여유공간 있는지 조사
				...
				if (_slotRemainSpace >= amount) { // 여유공간이 옮길 아이템 수량보다 많으면,
					slots[_sameItemIndex].Amount += amount; // 전부 그대로 옮김
				} else { // 여유공간이 부족하면, 일단 그 슬롯은 다 채우고 새로운 빈 슬롯을 탐색
			...
			} else { // 같은 종류의 아이템 존재하지 않을 경우
				_final_leftover_amount = Add_Item_To_Empty_Slot(itemProp, amount);
			}
		} else { // index를 지정한 경우 (이 경우, 수량이 초과되어도 빈 공간에 넣지 않음)
			if (slots[index].Data.ID == 0) { // 슬롯이 비어있으면, 그대로 할당(최대 max_amount)
				...
				if (_slotRemainSpace >= amount) { // 슬롯의 여유공간이 충분할 경우
	...
	/// <summary> 인벤토리의 지정한 index로 아이템 이동.<br/>
	/// index 지정하지 않으면 아이템 자동 이동 : 수량이 max가 아닌 중복 아이템 
	/// 탐색, 있으면 거기 추가하고 없으면 빈 슬롯에 할당.<br/>
	/// amount 지정하지 않으면 아이템 전체 이동 </summary>
	public void Move_Item_To_Inventory(ref ItemProp itemProp, int index = -1) {
		if (index == -1) { // index 지정하지 않으면, 같은 종류의 아이템 탐색
			int _sameItemIndex = Find_Index_Of_Same_Item(itemProp.Data);
			if (_sameItemIndex != -1) { // 같은 종류의 아이템 존재하면, 그 슬롯에 여유공간 있는지 조사
				int _slotRemainSpace = slots[_sameItemIndex].Data.Max_amount - slots[_sameItemIndex].Amount;
				if (_slotRemainSpace >= itemProp.Amount) { // 여유공간이 옮길 아이템 수량보다 많으면
					slots[_sameItemIndex].Amount += itemProp.Amount; // 전부 그대로 옮김
				} else { // 여유공간이 적으면, 그 슬롯은 다 채우고 새로운 빈 슬롯을 탐색
